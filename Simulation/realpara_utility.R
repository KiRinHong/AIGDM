...
Compo_glasso <- function(x, rho.list, option = 2, offset = (K + 1), para_NR = FALSE, num_cores_NR = 10, z_ratio = 1000, O_ratio = 1000, max_iter = 50)  
{
  gc()
  library(MASS)
  library(glasso)
  ...
  results <- list(z.end, Sigma.1, Omega.1, Omegas.1, rho.list.1)
  names(results) <- c("Z.mat", "Sigma.1", "Omega.1", "Omegas.1", "rho.list.1")
  return(results)
}