...
order_char <- commandArgs(trailingOnly = TRUE)
...
rslt.GDMC <- AIGDM.Cluster(id, Y.m, X.m, X.m, X.m, X.index,
                           Tax = tax, min.depth = 0, order = order_char, ZI.pos = "no", zi.check.pval.lst = NULL,
...
rslt.AIGDMC <- AIGDM.Cluster(id, Y.m, X.m, X.m, X.m, X.index,
                             Tax = tax, min.depth = 0, order = order_char, ZI.pos = "adaptive", zi.check.pval.lst = NULL,
                             n.boot = n.boot, n.cores = detectCores()/2, 
                             fdr.alpha = fdr.alpha, n.perm = n.perm, seed = seed)
...
rslt.ZIGDMC <- AIGDM.Cluster(id, Y.m, X.m, X.m, X.m, X.index,
                             Tax = tax, min.depth = 0, order = order_char, ZI.pos = "all", zi.check.pval.lst = NULL,
                             n.boot = n.boot, n.cores = detectCores()/2, 
                             fdr.alpha = fdr.alpha, n.perm = n.perm, seed = seed)
...
rslt.DMC <- DM.Cluster(ID = id, Y = Y.m, case = exposure, Tax = tax, min.depth = 0, order = order_char, 
                       fdr.alpha = fdr.alpha, perm.type = "BTW", n.perm = n.perm, seed = seed)
print("#### DMC done ####")
rslt.Ecam.full.delivery.woconf <- list(AIGDMC = rslt.AIGDMC, GDMC = rslt.GDMC, ZIGDMC = rslt.ZIGDMC, DMC = rslt.DMC)
save(rslt.Ecam.full.delivery.woconf, file = paste0("ECAM.full.rslt.delivery.woconf.omni.", order_char, ".rda"))
...
# print("#### GDMC test done ####")
# 
# load("ECAM.full.rslt.diet.woconf.LBFGS.genus.rda")
# zi.check.pval.lst <- rslt.Ecam.full.diet.woconf.testzero$AIGDMC.check$lineage.zi.pval
# rslt.AIGDMC <- AIGDM.Cluster(id, Y.m, X.m, X.m, X.m, X.index,
...
#                        fdr.alpha = fdr.alpha, perm.type = "BTW", n.perm = n.perm, seed = seed)
# print("#### DMC done ####")
# rslt.Ecam.full.diet.woconf <- list(AIGDMC = rslt.AIGDMC, GDMC = rslt.GDMC, ZIGDMC = rslt.ZIGDMC, DMC = rslt.DMC)
# save(rslt.Ecam.full.diet.woconf, file = "ECAM.full.rslt.diet.woconf.omni.rda")
